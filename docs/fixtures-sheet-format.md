...
Lists all divisions and flags group stages, the league/competition they are in, and other info, specifically the list of teams competing. If you introduce a new league, division, or group you may need to [add a competition for it](fixtures-tables-history.md#competitions-and-groups).
...
Flags group stages work the same as divisions so are also entered here, except the `League` column should be the Flags name e.g. "Senior Flags", and the Division should be the group name, usually something like A, B, C, D for a single group ground.

You must have the following columns in this order:

* `League` - currently SEMLA or Local, or Flags competition for group stages
* `Division` - within the league, so for "Local London 1" this would be "London 1", or flags group name
* `Full` - full name, just set this column to concatenate the previous 2, so `=A2&" "&B2` and copy down
* `# Promoted` - this and Relegated add bars to the league tables, and also tie break rules will apply got teams equal on points around both lines
* `# Relegated`
...
Team names must match those on the Teams sheet. **If you don't know the teams**, e.g. for flags group stages, then you can enter a name starting `TBD` as those won't be validated. Note that you cannot use duplicate team names within a division, so you should put `TBD 1`, `TBD 2`, etc., though you can use these again for other divisions.
...
The `Competition` for league matches must be the League and Division, so if the league is "SEMLA" and the Division is "Premier Division", then the value must be "SEMLA Premier Division". For Flags matches use the full flags competition with the round name abbreviation, so "Senior Flags R16", "Minor Flags F", "Minor Flags Play-In P1". Note that these will be correctly generated by the [Flags Fixtures Formulas](#adding-flags-fixtures).
...
Once a season is set up, and dates are set up on the Flags sheet, then you can add Flags matches to the main Fixtures sheet with the teams and scores automatically added from the Flags sheet. Once the Fixtures sheet has been imported, use the Flags Fixtures Formulas tab on the SEMLA WordPress Admin page to get the formulas to paste.
...
Fixtures Secretaries have a tendency to create their own style of fixtures spreadsheets. To easily convert to the format used by our website there is a [program to extract that data](../src/php/extract-fixtures.php). You will need to customise it to fit the supplied data, see the top of the program for details. If the spreadsheet format from the Secretary has changed dramatically you should check the Git file history to see if there is a relevant prior version.

Note that this program will also input the [Flags formulas](#adding-flags-fixtures) so that it can sort the flags matches into the correct position in the fixtures, which should save some effort.
...
The flags competitions should be in the order which matches the sequence specified in the database. This is basically Senior, Intermediate, then Minor, with any prelims/play-ins before the relevant main competition.

Group stages get added to the Divisions sheet.

Team names must match the name in the Teams sheet or be blank, or if the team is currently unknown you can also have the name starting "TBD", "Winner", "Runner-up", or "Loser", to denote who the team will be (note these are case sensitive).
...
For the grid of matches, the first round should have no gaps between matches, and column B needs H/A markers for Home or Away team, or NH and NA for neutral venues, as one team is still "home". Column A can have match numbers, but these aren't used.
...
Flags matches also need to be added to the Fixtures sheet so that each match can have a custom date, time, or venue, and they may also be rearranged. This isn't done automatically, so you need to follow [the instructions to add Flags matches to the main Fixtures sheet](#adding-flags-fixtures).
